...
preprocessing<-function(mat,meta,subject_var,group_var,sub_cell_filtering,gene_sub_filtering,gene_cell_filtering){
  ...
  stopifnot("Gene filtering should be based on an indicated proportion"=(sub_p_filtering>=0&sub_p_filtering<=1))
  stopifnot("Gene filtering should be based on an indicated proportion"=(gene_p_filtering>=0&gene_p_filtering<=1))
  ...
  remain_gene<-rownames(mat)[which(rowSums(sapply(group_sub_list,function(sub_list){
    sum(sub_prop[,sub_list]>=gene_cell_filtering,na.rm=T)>=(gene_sub_filtering*length(sub_list))
  }))>0)]
  return(mat=mat[remain_gene,remain_cell],meta=meta)
}
...
iDESC<-function(mat,meta,subject_var,group_var,zp_group_var,norm_opt=c("SeqDepth","SizeFactor","User","None"),user_sf=NULL,
  ...
  res_tb<-Reduce(plyr::rbind.fill,lapply(1:nrow(mat),function(g){
    gene<-mat[g,]
    model_select<-rep("Two Pi Model",nrow(mat))
    if(diagnosis){
      model_select<-zp_diagnosis(gene,zp_group_var,zp.thresh)
    }
    ...
    if(model_select=="One Pi Model"){
      f1<-try(glmmTMB::glmmTMB(y ~ group + offset(log(norm_sf)) + (1 | sub), data = tmp.df, family = nbinom2, zi = ~ 1))
      ...
      if('try-error' %in% class(res1)){
        ...
        names(res1)<-c("Alpha",paste0("Beta_",names(summary(f1)$coefficients$cond[,"Estimate"])[-1]), "Dispersion","Sigma",
                       ...
                       paste0("P-Beta_",names(summary(f1)$coefficients$cond[,"Estimate"])[-1]),"Convergence")
      }
    }else{
      f1<-try(glmmTMB::glmmTMB(y ~ group + offset(log(norm_sf)) + (1 | sub), data = tmp.df, family = nbinom2, zi = ~ zp_group))
      res1<-try(c(summary(f1)$coefficients$cond[,"Estimate"],1/summary(f1)$sigma,exp(f1$fit$par["theta"]),
                  boot::inv.logit(c(summary(f1)$coefficients$zi[1,"Estimate"],summary(f1)$coefficients$zi[-1,"Estimate"]+summary(f1)$coefficients$zi[1,"Estimate"])),summary(f1)$coefficients$cond[-1,"Pr(>|z|)"],
                  summary(f1)$coefficients$zi[-1,"Pr(>|z|)"],f1$fit$convergence))
      if('try-error' %in% class(res1)){
        ...
        names(res1)<-c("Alpha",paste0("Beta_",names(summary(f1)$coefficients$cond[,"Estimate"])[-1]), "Dispersion","Sigma",
                       paste0("Pi_",names(summary(f1)$coefficients$zi[,"Estimate"])),
                       paste0("P-Beta_",names(summary(f1)$coefficients$cond[,"Estimate"])[-1]),
                       paste0("P-Pi_",names(summary(f1)$coefficients$zi[,"Estimate"])[-1]),"Convergence")
      }
    }
    data.frame(t(res1))
  }))
  rownames(res_tb)<-rownames(mat)
  res_tb<-na.omit(res_tb[res_tb$Convergence==0,])
  return(res_tb)
}
